...
void AMyProjectCharacter::Tick(float DeltaTime)
{
	...
	//LineTrace from line 92 to line 116
	FHitResult OutHit;

	FVector Start = FP_Gun->GetComponentLocation();
	FVector ForwardVector = FirstPersonCameraComponent->GetForwardVector();
	FVector End = (Start + (ForwardVector * 1000.f)); //DebugLine
	
	float DP = FVector::DotProduct(ForwardVector,End);

	FVector ReflectingVector = (ForwardVector - 2 * (DP * ForwardVector));
	...
	if (isHit)
	{
		if (OutHit.bBlockingHit)
		{
			if (GEngine)
			{
				DrawDebugLine(GetWorld(), End, ReflectingVector, FColor::Red, false, 1, 0, 1);
